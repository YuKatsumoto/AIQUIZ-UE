...
### 1.17 ⭐ Importing a SKELETAL mesh / animation FBX (Mixamo etc.) — skin once, anims onto that skeleton
Same crash family as §1.15 (FBX import) → use the **defer-to-slate-post-tick** pattern there. Skeletal specifics:
- **Symptom:** re-importing each Mixamo clip "With Skin" spawns a duplicate SkeletalMesh + Skeleton +
  materials every time, and animations on the new skeleton won't share with the first character.
- **Cause:** a "With Skin" FBX always creates its own skeleton; an AnimSequence is bound to ONE skeleton.
- **Fix:** import the **character once** (`import_as_skeletal=True`, `mesh_type_to_import=FBXIT_SKELETAL_MESH`),
  then import every other clip **animation-only onto that skeleton**:
  ```python
  ui = unreal.FbxImportUI()
  ui.set_editor_property("skeleton", existing_skeleton)   # bind to the already-imported skeleton
  ...
  ```
  Download those extra clips **"Without Skin"** from Mixamo (small files, anim only). One AssetImportTask
  per clip; you can pass several tasks in a single `import_asset_tasks([...])` inside the deferred callback.

### 1.18 ⭐ Imported run/jump anim DRIFTS forward then snaps back ("moonwalk reset") every loop
- **Symptom:** a SequencePlayer-driven clip slides the whole character forward over the loop, then teleports
  back — looks broken for an in-place locomotion state.
- **Cause:** forward translation is baked into the clip's root track; without root-motion processing the bones
  carry it visually. (Mixamo clips not exported "In Place" do this.)
- **Fix (no re-export needed):** lock the root on the **AnimSequence asset**:
  ```python
  ...
  ```
  Do this for run/idle. For a **jump**, leave `force_root_lock=False` — the vertical hop usually lives in the
  hips, and locking the root can flatten it; an In-Place jump has no horizontal drift anyway.

### 1.19 ⭐ AnimBP authoring — wire the state machine to Output Pose, and feed vars in `BlueprintUpdateAnimation`
- A state machine built with `AnimGraphService.build_state_machine(...)` is **NOT auto-connected** to the
  AnimGraph result. Symptom: T-pose / nothing plays. Fix: `connect_to_output_pose(abp, "AnimGraph", machine_node_id)`
  (get the id from `list_state_machines`). Set per-state clips with `set_state_animation(...)` (creates+wires
  the SequencePlayer in one call). `validate_state_machine(...)` flags inert transitions (no rule → never fires)
  and unconnected states. Single looping clip → a 1-state machine is fine.
- **Locomotion vars should be read by the AnimBP itself**, not pushed from the Character. Override
  `BlueprintUpdateAnimation` and compute: `TryGetPawnOwner → GetVelocity → VSize → set Speed`; for airborne,
  `Cast to Character → CharacterMovement (member-get) → IsFalling → set bIsInAir`. In an AnimBP context
  `add_function_call_node("Character","GetCharacterMovement")` **fails** — use
  `add_member_get_node(abp, g, "Character", "CharacterMovement", x, y)` off the cast's `As…` pin instead.

### 1.20 ⭐ Can't drive AnimBP vars on a live PIE instance to test states → flip the variable DEFAULT instead
- **Symptom:** `anim_instance.set_editor_property("Speed", 500)` on a PIE instance raises *"cannot be edited on
  instances"* (§1.10), so you can't poke a state to verify it.
- **Fix:** with **PIE stopped**, set the variable's edit-time default, recompile, restart PIE — the state machine
  enters that branch immediately so you can screenshot it:
  `BlueprintService.set_variable_default_value(abp, "Speed", "500.0")` (then `compile_blueprint` + PIE). Restore
  defaults (`"0.0"` / `"false"`) when done. Reading a live instance var still works; only **writing** is blocked.

### 1.21 ⭐ Character化: set the inherited Mesh + camera on the template, GameMode via CDO, WorldSettings via the world
Make a `Character`-parented BP (`create_blueprint(name,"Character",folder)`). Components: root `CollisionCylinder`
(capsule), `CharacterMesh0` (the Mesh), `CharMoveComp`.
- **Inherited Mesh skeletal-mesh/anim-class won't set via `set_component_property`** (class/asset props, §1.10).
  Set them on the **template subobject** (§1.5 gather): `mesh_tmpl.set_editor_property("skeletal_mesh_asset", sk)`,
  `set_editor_property("anim_class", abp_generated_class)`. Mixamo mesh sits feet-at-origin facing +Y → on the
  template set `relative_location=(0,0,-90)` and `relative_rotation=Yaw -90` so feet hit the capsule bottom and
  it faces +X.
- Components added with `add_component` read back as **`<Name>_GEN_VARIABLE`** when gathered via
  SubobjectDataSubsystem — match by stripping that suffix.
- Boolean component props drop the `b` (SpringArm lag is **`enable_camera_lag`**, not `b_enable_camera_lag`).
- Character facing: on the CDO set `use_controller_rotation_yaw=False` and on `CharMoveComp` set
  `orient_rotation_to_movement=True` so the body turns toward movement.
- **GameMode default pawn** persists via the CDO (§1.10): `get_default_object(gm.generated_class())
  .set_editor_property("default_pawn_class", char_class)` → **`compile_blueprint` + `save_asset`** (or PIE keeps
  the engine default pawn).
- **Per-level GameMode override:** `WorldSettings` is NOT in `EditorActorSubsystem.get_all_level_actors()`.
  Get it from the world: `ues.get_editor_world().get_world_settings().set_editor_property("default_game_mode", gm_class)`,
  then `save_current_level()`.

### 1.22 ⭐ Enhanced Input: real modifier type names, and AddMappingContext needs a subsystem you CAN'T spawn directly
- **Modifier/trigger names differ from the skill prose.** `add_modifier(imc, idx, "SwizzleInputAxisValues")`
  returns **False** — the real name is **`SwizzleAxis`**. Always confirm via `get_available_modifier_types()` /
  `get_available_trigger_types()` (e.g. `Negate`, `Scalar`, `DeadZone`, `SwizzleAxis`). For a WASD→Axis2D Move:
  W=`SwizzleAxis` (X→Y), S=`Negate`+`SwizzleAxis`, A=`Negate`, D=none.
- **The "Get …LocalPlayerSubsystem" graph node is unspawnable via tooling.** `K2Node_GetSubsystemFromPC` /
  `K2Node_GetSubsystem` spawn, but their **`CustomClass` is a protected UPROPERTY** (can't set via
  `set_editor_property`, can't reach the node's pins), so the generic node returns **null at runtime** and
  `discover_nodes` never surfaces the typed accessor. **Fix:** call the BlueprintCallable library function and
  cast the result to the concrete subsystem (the cast types the value for AddMappingContext's interface pin):
  ```
  GetPlayerController(0) ─► SubsystemBlueprintLibrary::GetLocalPlayerSubSystemFromPlayerController
       (set Class pin = /Script/EnhancedInput.EnhancedInputLocalPlayerSubsystem)
    ─► add_cast_node "EnhancedInputLocalPlayerSubsystem" ─► (As… pin) ─► EnhancedInputLocalPlayerSubsystem::AddMappingContext.self
  ```
  Wire this off `Event BeginPlay`; set the AMC `MappingContext` pin to the IMC asset path and `Priority` 0.
  `EnhancedInputDeveloperSettings` (the "Default Mapping Contexts" auto-apply) is **not exposed in Python**, so
  the BeginPlay route is required.
- **`set_node_pin_value` on a CLASS pin reports `default_value=""` but DOES set it for a *function-parameter*
  class pin** (the value lives in `DefaultObject`, not the string) — verified working at runtime. It does **not**
  retype the output of the special `K2Node_GetSubsystem*` nodes, which is the separate bug above.
- `add_cast_node`'s success output pin is **localized** (`As<ClassName>`, e.g. `Asキャラクター`) — find it with
  `startswith("As")`, don't hard-code it.

### 1.23 ⭐ Graph node internals are mostly unreachable from Python — don't try to set protected K2 node fields
`BlueprintEditorLibrary.find_event_graph(bp)` returns the EdGraph, and `unreal.find_object(graph,
"K2Node_Foo_0")` can fetch a node object — but `EdGraph.nodes` and most `K2Node` UPROPERTYs (`CustomClass`,
`Pins`) are **protected** (no read/write, no reflectable setters). So you can't fix a node by poking its
fields; choose a node form that's correct on spawn (e.g. §1.22's library-call + cast) instead. `discover_nodes`
also returns **nothing for Subsystem / plugin (EnhancedInput) accessor nodes** — don't depend on it for those.

### 1.24 ⭐ Verifying input/possession headlessly — most PlayerController input hooks aren't exposed; use a log probe
You usually **cannot** simulate keys or read the live input subsystem from Python: `PlayerController` has no
`input_key` / `get_local_player`, its `Player` prop is protected, and `SubsystemBlueprintLibrary` /
`EnhancedInputDeveloperSettings` aren't Python modules. Flawed proxies: `launch_character` velocity decays
within a frame or two (you'll read `|v|=0`), and `call_method("Jump")` sets a flag that's consumed the same
frame so it often doesn't stick. **What works:**
- **Possession/anim:** PIE on, `GameplayStatics.get_player_controller(world,0).get_controlled_pawn()` → confirm
  class, read `mesh.get_anim_instance()` + its `Speed`/`bIsInAir` (read-only is allowed on instances).
- **State transitions:** prove them with the §1.20 default-value flip (Speed=500 → Run, bIsInAir=true → Jump).
- **IMC actually applied / a BeginPlay branch ran:** drop a temporary `KismetSystemLibrary.PrintString` on the
  exec path (and one on the cast-failed path), PIE, then grep `Saved/Logs/*.log` for
  `LogBlueprintUserMessages: [<Actor>] <marker>`. Presence of the success marker + absence of the failure
  marker is definitive; remove the probes afterward.
