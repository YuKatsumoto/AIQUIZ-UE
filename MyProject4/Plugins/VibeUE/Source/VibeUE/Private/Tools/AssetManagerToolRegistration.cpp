...
static TMap<FString, FString> FlattenParams(const TMap<FString, FString>& Params)
{
	...
	for (const auto& Pair : JsonObj->Values)
	{
		...
		{
			Flat.Add(FString(*Pair.Key),Pair.Value->AsString());
		}
		else if (Pair.Value->Type == EJson::Boolean)
		{
			Flat.Add(FString(*Pair.Key),Pair.Value->AsBool() ? TEXT("true") : TEXT("false"));
		}
		else if (Pair.Value->Type == EJson::Number)
		{
			Flat.Add(FString(*Pair.Key),FString::Printf(TEXT("%.10g"), Pair.Value->AsNumber()));
		}
		else
		{
			FString ValStr;
			...
			Flat.Add(FString(*Pair.Key),ValStr);
		}
	}
	return Flat;
}
